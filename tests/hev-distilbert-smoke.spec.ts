...
async function maybeSetHevEnvFromBase(): Promise<void> {
  ...
  if (!(typeof process.env.HEV_DISTILBERT_ONNX_PATH === 'string' && process.env.HEV_DISTILBERT_ONNX_PATH.trim().length > 0)) {
    const model = path.resolve(base, 'hev', 'model.quant.onnx');
    if (fs.existsSync(model)) process.env.HEV_DISTILBERT_ONNX_PATH = model;
  }

  if (!(typeof process.env.HEV_DISTILBERT_VOCAB_PATH === 'string' && process.env.HEV_DISTILBERT_VOCAB_PATH.trim().length > 0)) {
    const vocab = path.resolve(base, 'hev', 'vocab.txt');
    if (fs.existsSync(vocab)) process.env.HEV_DISTILBERT_VOCAB_PATH = vocab;
  }
}

void maybeSetHevEnvFromBase();
